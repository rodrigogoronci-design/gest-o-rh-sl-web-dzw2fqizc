import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Wallet } from 'lucide-react'

export default function ContraCheque() {
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Contra Cheque</h1>
          <p className="text-muted-foreground">
            Visualize e baixe seus demonstrativos de pagamento mensais.
          </p>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Wallet className="w-5 h-5 text-primary" />
            Em construção
          </CardTitle>
          <CardDescription>
            A funcionalidade de contra cheque será trabalhada e disponibilizada em breve.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[300px] flex flex-col items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground bg-muted/20">
            <Wallet className="w-12 h-12 mb-4 text-muted-foreground/50" />
            <p className="text-lg font-medium">Área de Demonstrativos</p>
            <p className="text-sm">
              Aqui você terá acesso a todos os seus holerites e informes de rendimento.
            </p>
          </div>
        </CardContent>
      </Card>
    </div>
  )
}
